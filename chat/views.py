...
from administrator.all_models import TrainingStudent, DepartmentTrainingCoordinator
User = get_user_model()
...
def request_to_change_acceptance_letter(request):
    if request.user.is_student:
        ...
        msg_req.save()
        messages.success(
            request, f'Your request to change your {student.level}, level acceptance letter has been sentto your coordinator')
        return redirect(reverse('chat:notification'))
    return False
...
def approve_request(request, msg_id):
    if request.user.is_coordinator:
        ...
        msg.is_approved_request = True
        msg.save()
        messages.success(
            request, f'Your you just approved a request of {msg.from_sender.first_name}, to change his/her acceptance letter')
        return redirect(reverse('chat:notification'))
    return False
...
def sendMessage(request, user_id):
    ...
    my_friend = User.objects.get(identification_num=user_id)
    # Here we filter the messages that was sent to the current user, but yet he/she didn't view (read) it
    unread_msg = Message.objects.filter(to_receiver=request.user.id, is_view=False)
        ...
        if m_form.is_valid():
            ...
            try:
                get_msgs_1 = Message.objects.filter(
                    from_sender=request.user.id, to_receiver=my_friend).last()
                if get_msgs_1.from_sender == request.user:
                    ...
                    is_new_msg = Message.objects.filter(
                        from_sender=request.user.id, to_receiver=my_friend, is_msg_added=False, is_new_message=False) | Message.objects.filter(from_sender=request.user.id, to_receiver=my_friend, is_msg_added=True, is_new_message=False)
            ...
            try:
                get_msgs_2 = Message.objects.filter(
                    to_receiver=request.user.id, from_sender=my_friend).last()
                if get_msgs_2.to_receiver == request.user:
                    # This is if the message is replied
                    is_replied_msg = Message.objects.filter(
                        to_receiver=request.user.id, from_sender=my_friend, is_msg_replied=False)
