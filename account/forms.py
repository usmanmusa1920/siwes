...
class AdministratorSignupForm(UserCreationForm):
    ...
    description = forms.CharField(
        required=True)  # it is not part of User model fields

    class Meta:
        model = User
        fields = ['first_name', 'middle_name', 'last_name', 'gender', 'date_of_birth',
                  'identification_num', 'email', 'phone_number', 'country', 'password1', 'password2', 'description']
...
class FacultySignupForm(forms.ModelForm):
    ...
    class Meta:
        model = Faculty
        fields = ['training', 'name', 'email',
                  'website', 'phone_number', 'description']


class FacultyDeanSignupForm(UserCreationForm):
    """faculty dean signup form class"""
    ranks = forms.CharField(
        required=True)  # it is not part of User model fields

    class Meta:
        model = User
        fields = ['first_name', 'middle_name', 'last_name', 'gender', 'date_of_birth',
                  'identification_num', 'email', 'phone_number', 'country', 'password1', 'password2', 'ranks']
...
class DepartmentCoordinatorSignupForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ['first_name', 'middle_name', 'last_name', 'gender', 'date_of_birth',
                  'identification_num', 'email', 'phone_number', 'country', 'password1', 'password2']
...
class StudentSignupForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ['first_name', 'middle_name', 'last_name', 'gender', 'date_of_birth', 'identification_num',
                  'email', 'phone_number', 'country', 'password1', 'password2', 'student_level']
