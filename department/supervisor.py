...
from toolkit.decorators import (supervisor_required)
from administrator.tables import (
    Session, Coordinator, Supervisor, Student, Acceptance, WeekEntry, Result
)
...
def submit_student_logbook(request, matrix_no):
    ...
    # checking result if already been released
    if is_result:
        messages.warning(
    ...
    def train_calc():
        # fetching student week entry so that to loop over them by incrementing each week grade (summing them all)
    ...
    # once supervisor submit student logbook, that student will be remove from the list of that supervisor student, so that in the next session he will be assign with different student base on the next coordinator decision.
