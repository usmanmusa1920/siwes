...
class TrainingStudent(models.Model):
    ...
    student_training_coordinator = models.ForeignKey(
    ...
    level = models.CharField(
        max_length=100, default='200', choices=level_choices)
...
class StudentLetterRequest(models.Model):
    ...
    sender_req = models.ForeignKey(
        TrainingStudent, related_name='sender_req', on_delete=models.CASCADE)
    receiver_req = models.ForeignKey(
        DepartmentTrainingCoordinator, related_name='receiver_req', on_delete=models.CASCADE)
...
class AcceptanceLetter(models.Model):
    sender_acept = models.ForeignKey(
        TrainingStudent, related_name='sender_acept', on_delete=models.CASCADE)
    receiver_acept = models.ForeignKey(
        DepartmentTrainingCoordinator, related_name='receiver_acept', on_delete=models.CASCADE)
    timestamp = models.DateTimeField(default=timezone.now)
    ...
    image = models.ImageField(blank=True, null=True,
                              upload_to=f'acceptance-letters')
...
class WeekReader(models.Model):
    student = models.ForeignKey(
        TrainingStudent, related_name='student_week_reader', on_delete=models.CASCADE)
...
class WeekScannedLogbook(models.Model):
    week = models.ForeignKey(
        WeekReader, related_name='week_reader', on_delete=models.CASCADE)
    student_lg = models.ForeignKey(
        TrainingStudent, related_name='student_lg', on_delete=models.CASCADE)
    timestamp = models.DateTimeField(default=timezone.now)
    title = models.CharField(max_length=255, blank=True, null=True)
    week_no = models.IntegerField(blank=True, null=True)
    # avoid updating student acceptance letter on admin page
    image = models.ImageField(blank=True, null=True,
                              upload_to=f'weekly-scanned-logbook')
...
class CommentOnLogbook(models.Model):
    ...
    logbook = models.ForeignKey(
        WeekScannedLogbook, on_delete=models.CASCADE)  # Week scanned logbook
